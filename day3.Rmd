...
We want to predict whether respondents over-estimate immigration from non-western contexts. We begin by looking at the distribution of the dependent variable. We check how well we could predict misperception of immigration in our sample without a statistical model.
...
m.log <- glm(over.estimate ~ RSex + RAge + Househld + Lab + SNP + Ukip + BNP + 
               ...
               employMonths + urban + health.good + HHInc, data = df,
...
m.log <- glm(over.estimate ~ RSex + RAge + Househld + Lab + SNP + Ukip + BNP + 
               GP + party.other + paper + WWWhourspW +  religious + 
               employMonths + urban + health.good + HHInc, data = df, subset = train.ids, 
             family = binomial(link = "logit"))
...
Let's try to improve the classification model by relying on the strongest predictors.

```{r}
# try to improve the prediction model by relying on strong predictors
m.log <- glm(over.estimate ~ RSex + urban + HHInc, 
...
```{r}
# fit model
m.lda <- lda( over.estimate ~ RSex + urban + HHInc, data = df, subset = train.ids)
m.lda
...
train.X <- cbind( df$RSex, df$urban, df$HHInc )[train.ids, ]
test.X <- cbind( df$RSex, df$urban, df$HHInc )[-train.ids, ]
...
knn.out <- knn(train.X, test.X, train.Y, k = 11)
mean( knn.out == df.test$over.estimate )
```
...
m.lm <- glm(IMMBRIT ~ RSex + urban + HHInc, data = df, subset = train.ids, family = gaussian())

# predictions
beta_hats <- as.matrix(coef(m.lm))
X <- cbind( 1, 
            df.test$RSex == "Female", 
            df.test$urban == "more rural",
            df.test$urban == "more urban",
            df.test$urban == "urban",
            df.test$HHInc)

# yhats (y_hat = X * beta_hats)
y_hats <- X %*% beta_hats

# now we do the classfication 
exp.out <- ifelse( y_hats > 10.7, yes = 1, no = 0)
...
```
  ...
  then a plot should be created with an $x$-axis taking on values $1,2,.,10$ and a $y$-axis taking on values
  $1^3,2^3,.,10^3$.
