...
|#|Description|
|-----|------------------------------------------------------------------|
|![](../img/circle1.png)|The *dependent* variable, also sometimes called the outcome variable. We are trying to model the effects of `NoHighSchool` on `UnemploymentRate` so `UnemploymentRate` is the *dependent* variable.|
|![](../img/circle2.png)|The *independent* variable or the predictor variable. In our example, `NoHighSchool` is the *independent* variable.|
|![](../img/circle3.png)|The differences between the observed values and the predicted values are called *residuals*.|
...
|![](../img/circle5.png)|The *p-value* of the model. Recall that according to the null hypotheses, the coefficient of interest is zero. The *p-value* tells us whether can can reject the null hypotheses or not.|
|![](../img/circle6.png)|The *standard error* estimates the standard deviation of the coefficients in our model. We can think of the *standard error* as the measure of precision for the estimated coefficients.|
|![](../img/circle7.png)|The *t statistic* is obtained by dividing the *coefficients* by the *standard error*.|
|![](../img/circle8.png)|The *R-squared* and *adjusted R-squared* tell us how much of the variance in our model is accounted for by the *independent* variable. The *adjusted R-squared* is always smaller than *R-squared* as it takes into account the number of *independent* variables and degrees of freedom.|
